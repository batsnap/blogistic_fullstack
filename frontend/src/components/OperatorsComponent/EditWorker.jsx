...
class EditWorker extends React.Component {
	...
	componentDidMount() 
	{
		check_token();
		setTimeout(()=>{
			axios.get("http://localhost:8000/api/Workers/" + window.location.pathname.split('/')[2] + '/',
				{
	...
	handleSubmit(event)
	{
		axios.post('http://localhost:8000/api/EditProfileWorker/',
		...
		window.location.replace('/ListWorkers')
	}
	onChange = (e) => {
		...
		return (
			<div className="row">
				...
				<div className="col-sm-4 bg-ligth text-black ">
					<center>
						<h3>Рабочий номер-{this.state.id_worker}</h3>
					</center>
					<table className="table">
                        ...
                        <tr>
							<td>Должность</td>
							<td><input style={{width:'400px'}} type="text" name='position' value={this.state.position} onChange={this.onChange}/></td>
						</tr>
						</tbody>
					</table>
					<center>
